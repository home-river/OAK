...
class TestConfigDTOSmokeTest:
    ...
    def test_device_config_dto_smoke(self):
        ...
        assert device_metadata.validate(), \
            f"设备配置验证失败: {device_metadata.get_validation_errors()}"
    ...
    def test_device_manager_config_dto_smoke(self):
        """冒烟测试：设备管理器配置DTO"""
        oak_hw_config = OAKConfigDTO()
        binding = DeviceRoleBindingDTO(
            role=DeviceRole.LEFT_CAMERA,
            historical_mxids=["test_mxid_12345"],
            active_mxid="test_mxid_12345",
            last_active_mxid="test_mxid_12345",
        )
        ...
        oak_module = OAKModuleConfigDTO(
            hardware_config=oak_hw_config,
            role_bindings={DeviceRole.LEFT_CAMERA: binding},
            device_metadata={"test_mxid_12345": metadata},
        )
