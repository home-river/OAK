...
class DisplayManager:
    ...
    def start(self) -> bool:
        ...
        2. 根据 enable_display 配置决定是否启动 DisplayRenderer
        3. 如果启动失败，清理已启动的子模块（需求 5.5）
    ...
    def stop(self, timeout: float = 5.0) -> bool:
        ...
        # 2. 停止 RenderPacketPackager（需求 14.2, 14.3, 14.5）
        if self._packager is not None:
            self.logger.info("停止 RenderPacketPackager...")
            try:
                # 传递超时参数（需求 14.6）
                self._packager.stop(timeout=timeout)
            except Exception as e:
                self.logger.error(
    ...
    def is_running(self) -> bool:
        ...
        检查两个子模块的运行状态。
        只有当 RenderPacketPackager 正在运行时，才认为模块在运行。
        DisplayRenderer 可能因为 enable_display=False 而未启动。
        
        Returns:
            bool: 是否正在运行
        """
        # 检查 RenderPacketPackager 是否正在运行
        packager_running = (
            self._packager is not None 
            and self._packager._running.is_set()
        )
        
        # 如果启用了显示，还需要检查 DisplayRenderer
        if self._config.enable_display:
            renderer_running = (
                self._renderer is not None 
                and self._renderer.is_running
            )
            return packager_running and renderer_running
        
        # 如果未启用显示，只检查 RenderPacketPackager
        return packager_running
