...
# 注意：移除了对 validators 的导入，因为实际未使用
# 验证逻辑应该由调用方（如 DeviceConfigManager）负责
...
def template_OAKConfigDTO() -> OAKConfigDTO:
    return OAKConfigDTO(
        ...
        preview_resolution=(512, 288),            # 预览帧分辨率（显示或加速用），匹配神经网络输入
...
def template_DeviceManagerConfigDTO(devices: List[DeviceMetadataDTO]) -> DeviceManagerConfigDTO:
    """
    构建 DeviceManagerConfigDTO 的默认模板。

    注意：此函数仅创建模板，不进行验证。
    调用方（如 DeviceConfigManager）负责验证配置的正确性。
