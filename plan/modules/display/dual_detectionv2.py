...
class DualOAKDetector:
    def __init__(self, config_file=current_path/"configs/dual_oak_config.json", enable_alert=False,enable_can=False, can_bitrate=250000):
        ...
        # 显示模式控制
        self.fullscreen_mode = True   # 默认全屏模式，适合香橙派自启动
        self.display_mode = "combined"  # "combined", "oak_left", "oak_right"
        
        # 分辨率配置 - 可直接修改这些参数来改变显示分辨率
        self.single_frame_width = 640    # 单个设备帧宽度 (改为1920可获得4K效果)
        self.single_frame_height = 360   # 单个设备帧高度 (改为1080可获得4K效果)
        self.fullscreen_width = 1920     # 全屏模式宽度
        self.fullscreen_height = 1080    # 全屏模式高度
        self.window_width = 1280         # 窗口模式宽度
        self.window_height = 360         # 窗口模式高度
        ...
        with self.frame_lock:
            ...
            for alias, frame_data in self.device_frames.items():
                if frame_data['rgb'] is not None and frame_data['depth'] is not None:
                    ...
                    depth_resized = cv2.resize(frame_data['depth'], (self.single_frame_width, self.single_frame_height))
            ...
            while self.running:
                ...
                if self.display_mode == "combined":
                    ...
                    if combined_rgb is not None:
                        # 根据全屏模式调整显示分辨率
                        if self.fullscreen_mode:
                            display_frame = cv2.resize(combined_rgb, (self.fullscreen_width, self.fullscreen_height))
                        else:
                            display_frame = cv2.resize(combined_rgb, (self.window_width, self.window_height))
