...
class CANCommunicator:
    ...
    def __init__(self, shared_data: dict, lock: threading.Lock, 
                 ...
                 auto_configure: bool = True, enable_alert: bool = True):
        """
        初始化CAN通信器的配置。
    ...
    def configure_can_interface(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(f"已设置 {self.channel} 类型为CAN")
            except subprocess.CalledProcessError as e:
                print(f"设置CAN类型失败: {e.stderr}")
                return False
            ...
            try:
                subprocess.run(['sudo', 'ip', 'link', 'set', self.channel, 'type', 'can', 
                             ...
                             check=True, capture_output=True, text=True)
                print(f"已设置 {self.channel} 波特率为 {self.bitrate}")
            except subprocess.CalledProcessError as e:
                print(f"设置波特率失败: {e.stderr}")
                return False
